...
def ftu_exact_statistics(X):
    s, ratio = ftu_exact_ratio(X)
    return s, ratio * 4


def quantile_exact(alpha, m, n_unif):
    Phi_n = []
    ...
    for _ in range(m):
        ...
        _, Phi = ftu_exact_statistics(U_b)
        Phi_n.append(Phi)
...
def folding_test_exact(X, alpha):
    if not isinstance(X, np.ndarray):
        X = np.array(X)
    assert X.ndim == 1 or X.shape[1] == 1,"X must be a 1d array."
    n = X.shape[0]
    s, ftu_stat = ftu_exact_statistics(X)
    q = quantile_exact(alpha, m=1000, n_unif=n)
    if ftu_stat >= q:
